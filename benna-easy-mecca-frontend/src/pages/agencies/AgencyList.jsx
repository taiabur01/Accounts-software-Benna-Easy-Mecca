import { useState, useEffect, useCallback } from "react";
import { fetchAgencies, deleteAgency } from "../../api/agencies";
import AgencyFormModal from "./AgencyFormModal";

export default function AgencyList() {
  const [data, setData] = useState(null); // Laravel paginator response
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState("");
  const [page, setPage] = useState(1);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAgency, setEditingAgency] = useState(null);

  const load = useCallback(() => {
    setLoading(true);
    fetchAgencies({
      search: search || undefined,
      agency_type: typeFilter || undefined,
      page,
      per_page: 15,
    })
      .then(setData)
      .catch(() => alert("Failed to load agencies. Is the backend running?"))
      .finally(() => setLoading(false));
  }, [search, typeFilter, page]);

  useEffect(() => {
    load();
  }, [load]);

  // Reset to page 1 whenever the search/filter changes
  useEffect(() => {
    setPage(1);
  }, [search, typeFilter]);

  const handleAdd = () => {
    setEditingAgency(null);
    setModalOpen(true);
  };

  const handleEdit = (agency) => {
    setEditingAgency(agency);
    setModalOpen(true);
  };

  const handleDelete = async (agency) => {
    if (!confirm(`Delete agency "${agency.agency_name}"?`)) return;
    try {
      await deleteAgency(agency.id);
      load();
    } catch (err) {
      alert(err.response?.data?.message ?? "Failed to delete agency.");
    }
  };

  const handleSaved = () => {
    setModalOpen(false);
    load();
  };

  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-[#2c3e50]">Agencies</h1>
        <button
          onClick={handleAdd}
          className="bg-[#34495e] text-white text-sm font-bold px-4 py-2 rounded hover:bg-[#2c3e50]"
        >
          + Add Agency
        </button>
      </div>

      <div className="flex gap-3 mb-4">
        <input
          type="text"
          placeholder="Search by name, contact, or phone..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#34495e]"
        />
        <select
          value={typeFilter}
          onChange={(e) => setTypeFilter(e.target.value)}
          className="border border-gray-300 rounded px-3 py-2 text-sm"
        >
          <option value="">All Types</option>
          <option value="BD">BD</option>
          <option value="SAUDI">SAUDI</option>
        </select>
      </div>

      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left text-[#2c3e50] text-xs uppercase">
              <th className="px-4 py-3">Agency Name</th>
              <th className="px-4 py-3">Type</th>
              <th className="px-4 py-3">Contact</th>
              <th className="px-4 py-3">Phone</th>
              <th className="px-4 py-3 text-right">Opening Balance</th>
              <th className="px-4 py-3 text-center">G-Codes</th>
              <th className="px-4 py-3 text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading && (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-center text-gray-400">
                  Loading...
                </td>
              </tr>
            )}
            {!loading && data?.data?.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-center text-gray-400">
                  No agencies found.
                </td>
              </tr>
            )}
            {!loading &&
              data?.data?.map((agency) => (
                <tr key={agency.id} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium text-[#2c3e50]">
                    {agency.agency_name}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-bold ${
                        agency.agency_type === "BD"
                          ? "bg-blue-100 text-[#2980b9]"
                          : "bg-red-100 text-[#c0392b]"
                      }`}
                    >
                      {agency.agency_type}
                    </span>
                  </td>
                  <td className="px-4 py-3">{agency.contact_person || "-"}</td>
                  <td className="px-4 py-3">{agency.phone || "-"}</td>
                  <td className="px-4 py-3 text-right">
                    {Number(agency.opening_balance).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                    })}{" "}
                    {agency.opening_balance_type}
                  </td>
                  <td className="px-4 py-3 text-center">{agency.g_codes_count}</td>
                  <td className="px-4 py-3 text-right space-x-2">
                    <button
                      onClick={() => handleEdit(agency)}
                      className="text-[#2980b9] hover:underline text-xs font-bold"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(agency)}
                      className="text-[#e74c3c] hover:underline text-xs font-bold"
                    >
                      Delete
                    </button>
                  </td>
                </tr>
              ))}
          </tbody>
        </table>
      </div>

      {data && data.last_page > 1 && (
        <div className="flex items-center justify-between mt-4 text-sm text-gray-500">
          <span>
            Showing {data.from}-{data.to} of {data.total}
          </span>
          <div className="flex gap-2">
            <button
              disabled={page <= 1}
              onClick={() => setPage((p) => p - 1)}
              className="px-3 py-1 border border-gray-300 rounded disabled:opacity-40"
            >
              Prev
            </button>
            <span className="px-2 py-1">
              Page {data.current_page} / {data.last_page}
            </span>
            <button
              disabled={page >= data.last_page}
              onClick={() => setPage((p) => p + 1)}
              className="px-3 py-1 border border-gray-300 rounded disabled:opacity-40"
            >
              Next
            </button>
          </div>
        </div>
      )}

      {modalOpen && (
        <AgencyFormModal
          agency={editingAgency}
          onClose={() => setModalOpen(false)}
          onSaved={handleSaved}
        />
      )}
    </div>
  );
}
