// src/pages/PurchaseEntryForm.jsx
//
// Confirmed against StorePurchaseRequest.php + PurchaseController.php:
//  - purchase_date, g_code_id: correct, unchanged (my earlier guess of
//    "purchase_date" turned out right).
//  - items[].amount removed — PurchaseController computes it via
//    ItemAmountCalculator, same as Invoice.
//  - items[].details now use the corrected itemTypes.js (sale_amount).
...
export default function PurchaseEntryForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Purchase submitted successfully.' });
      setGCode('');
      ...
      const text = errors
        ...
        : err.response?.data?.message || 'Failed to submit purchase.';
  ...
  return (
    <form
      ...
      className="bg-white rounded-lg shadow p-6 border-t-4 border-red-700 max-w-6xl mx-auto mb-10"
    >
      <h2 className="text-xl font-bold text-slate-800 border-b pb-2 mb-5">
        2. Entry Form - For Purchase (Cost / Payable)
      </h2>
      ...
      <table className="w-full border-collapse mb-4">
        <thead>
          <tr className="bg-gray-50 text-left text-xs text-slate-800">
            ...
            <th className="p-3 w-1/5">Purchase Amount (SAR)</th>
      ...
      <div className="text-right bg-gray-50 border border-gray-200 rounded p-4 mb-5">
        ...
        <span className="text-2xl font-bold text-red-700 ml-3">{total.toFixed(2)}</span>
      </div>
      ...
      <button
        type="submit"
        disabled={submitting}
        className="w-full bg-red-700 hover:bg-red-800 disabled:opacity-50 text-white font-bold text-base py-3 rounded"
      >
